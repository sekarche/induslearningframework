...
public class SumSuffStatForAllHierarchyQueryConstructor extends QueryConstructor {
   ...
   private static final String QUERY_WITH_SIMPLE_FEATURE =
      "SELECT " + HIERARCHY_VAR_PATTERN + " SUM(" + VALUE_VAR_PATTERN + ") " + CONTEXT_PATTERN + " WHERE { "
      + INSTANCE_TYPE + " "
   ...
   private static final String QUERY_WITH_AGGREGATION_FEATURE =
      "SELECT " + HIERARCHY_VAR_PATTERN + " SUM(" + AGGREGATION_VAR + ") " + CONTEXT_PATTERN + " WHERE { "
      + "{ "
   ...
   public SumSuffStatForAllHierarchyQueryConstructor(RDFDataDescriptor desc, String context, SuffStatQueryParameter queryParam) {
      super(desc, context);
      mParam = queryParam;
   }
